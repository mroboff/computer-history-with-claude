...
impl UsbDevice {
    ...
    ///
    /// Uses USB HID class (0x03) detection combined with product name heuristics
    /// since many USB keyboards report device_class as 0x00 (composite device).
    pub fn is_keyboard(&self) -> bool {
        // Check product name for keyboard indicators
        let name_lower = self.product_name.to_lowercase();
        let vendor_lower = self.vendor_name.to_lowercase();
        let combined = format!("{} {}", vendor_lower, name_lower);

        // Common keyboard name patterns
        ...
        // Check if it's HID class and likely a keyboard
        // USB HID class is 0x03
        if self.device_class == 0x03 && !self.is_mouse() {
            // HID device that's not clearly a mouse is likely keyboard
            return true;
        }
    ...
    /// Check if this device is a mouse or pointing device
    ///
    /// Uses USB HID class (0x03) detection combined with product name heuristics.
    pub fn is_mouse(&self) -> bool {
        ...
        // Common mouse/pointing device name patterns
        let mouse_patterns = [
            "mouse",
            "mice",
            "trackball",
    ...
    /// Check if this device is an input device (keyboard or mouse)
    ///
    /// For single-GPU passthrough, at least one keyboard and mouse are required
    /// since the display manager is stopped and there's no graphical session.
    pub fn is_input_device(&self) -> bool {
        self.is_keyboard() || self.is_mouse()
    }
...
mod tests {
    ...
    fn test_keyboard_detection() {
        // Test keyboard by name
        let keyboard = UsbDevice {
            vendor_id: 0x1234,
            product_id: 0x5678,
            vendor_name: "Generic".to_string(),
            product_name: "USB Keyboard".to_string(),
            bus_num: 1,
            dev_num: 2,
            device_class: 0,
        };
        ...
        assert!(keyboard.is_input_device());

        // Test mechanical keyboard by name
        let mech = UsbDevice {
            vendor_id: 0x1234,
            product_id: 0x5678,
            vendor_name: "Keychron".to_string(),
            product_name: "K2".to_string(),
            bus_num: 1,
            dev_num: 2,
            device_class: 0,
        };
        ...
        // Test HID class keyboard
        let hid_kb = UsbDevice {
            vendor_id: 0x1234,
            product_id: 0x5678,
    ...
    fn test_mouse_detection() {
        ...
        assert!(mouse.is_input_device());

        // Test gaming mouse by model
        let gaming = UsbDevice {
            vendor_id: 0x046d,
            product_id: 0xc08b,
            vendor_name: "Logitech".to_string(),
            product_name: "G502 HERO".to_string(),
            bus_num: 1,
            dev_num: 3,
            device_class: 0,
        };
    ...
    #[test]
    fn test_non_input_device() {
        let storage = UsbDevice {
            vendor_id: 0x0781,
            product_id: 0x5583,
            vendor_name: "SanDisk".to_string(),
            product_name: "Ultra Fit".to_string(),
            bus_num: 1,
            dev_num: 4,
            device_class: 0x00,
        };
